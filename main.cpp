...
search::Limits parseGo(const std::string& line, const board::Board& b) {
  search::Limits l;
  ...
  if (!l.infinite && l.movetimeMs <= 0) {
    int remain = b.whiteToMove ? wtime : btime;
    int inc = b.whiteToMove ? winc : binc;
    if (remain > 0) {
      int mtg = std::clamp(movestogo, 8, 60);
      int base = remain / mtg;
      int bonus = (inc * 7) / 10;
      int overhead = 20;
      int alloc = std::max(5, base + bonus - overhead);
      int hardCap = std::max(5, remain - 10);
      l.movetimeMs = std::min(alloc, hardCap);
    }
  }
  return l;
}
